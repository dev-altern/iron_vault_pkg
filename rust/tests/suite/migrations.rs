...
fn checksum_mismatch_blocks_subsequent_migrations() {
    ...
    let result = db.migrate(vec![
        m(
            1,
            "v1",
            "CREATE TABLE t1_MODIFIED (id TEXT PRIMARY KEY)",
            None,
        ),
...
fn sql_error_rolls_back_failing_migration() {
    ...
    let result = db.migrate(vec![m(1, "bad_sql", "THIS IS NOT VALID SQL", None)]);
    assert!(result.is_err());
    let msg = result.unwrap_err().to_string();
...
fn rollback_reverses_schema_changes() {
    ...
    let report = db.rollback_to(2, standard_migrations()).unwrap();
    ...
    assert!(
        result.is_err(),
        "tags table should not exist after rollback"
    );
...
fn rollback_missing_migration_definition_errors() {
    ...
    assert!(
        result.is_err(),
        "Should fail — missing migration definitions for v4/v5"
    );
}
...
fn stats_total_tables_excludes_internal_tables() {
    ...
    assert_eq!(
        ...
        "Internal _migrations table should be excluded"
    );
}
...
fn whitespace_only_difference_changes_checksum() {
    ...
    assert!(
        result.is_err(),
        "Internal whitespace change should change checksum"
    );
}
