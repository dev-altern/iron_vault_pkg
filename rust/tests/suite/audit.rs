...
fn write_audit_creates_entry() {
    ...
    assert_eq!(history[0].after_json.as_deref(), Some("{\"name\":\"Alice\"}"));
...
fn write_audit_uses_current_actor() {
    ...
    db.write_audit("users".into(), "r1".into(), "UPDATE".into(), None, None, None)
        .unwrap();
...
fn multiple_entries_for_same_row() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, Some("{}".into()), None)
        .unwrap();
    db.write_audit("users".into(), "r1".into(), "UPDATE".into(), Some("{}".into()), Some("{\"v\":2}".into()), None)
        .unwrap();
    db.write_audit("users".into(), "r1".into(), "DELETE".into(), Some("{\"v\":2}".into()), None, None)
        .unwrap();
...
fn get_actor_history_filters_by_actor() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, None, None).unwrap();
    db.write_audit("orders".into(), "r2".into(), "INSERT".into(), None, None, None).unwrap();

    db.set_actor("bob".into()).unwrap();
    db.write_audit("users".into(), "r3".into(), "INSERT".into(), None, None, None).unwrap();

    let alice_history = db.get_actor_history("alice".into(), None, None, 50).unwrap();
...
fn get_table_history_filters_by_table() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, None, None).unwrap();
    db.write_audit("users".into(), "r2".into(), "INSERT".into(), None, None, None).unwrap();
    db.write_audit("orders".into(), "r3".into(), "INSERT".into(), None, None, None).unwrap();

    let users_h = db.get_table_history("users".into(), None, None, 50).unwrap();
    assert_eq!(users_h.len(), 2);

    let orders_h = db.get_table_history("orders".into(), None, None, 50).unwrap();
...
fn get_history_respects_limit() {
    ...
    for i in 0..10 {
        db.write_audit("users".into(), "r1".into(), "UPDATE".into(), None, Some(format!("{{{}}}", i)), None).unwrap();
    }
...
fn get_history_empty_when_no_entries() {
    ...
    let h = db.get_history("users".into(), "nonexistent".into(), 50).unwrap();
    assert!(h.is_empty());
}
...
fn verify_integrity_clean_log() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, Some("{}".into()), None).unwrap();
    db.write_audit("users".into(), "r2".into(), "INSERT".into(), None, Some("{}".into()), None).unwrap();
...
fn verify_integrity_detects_tampered_actor() {
    ...
    db.execute_raw(
        format!("UPDATE _audit_log SET actor_id = 'hacker' WHERE id = '{}'", entry_id),
        vec![],
    )
    .unwrap();
...
fn verify_integrity_detects_tampered_data() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, Some("{\"name\":\"Alice\"}".into()), None).unwrap();
    let history = db.get_history("users".into(), "r1".into(), 1).unwrap();
    let entry_id = &history[0].id;

    // Tamper: modify after_json
    db.execute_raw(
        format!("UPDATE _audit_log SET after_json = '{{\"name\":\"EVIL\"}}' WHERE id = '{}'", entry_id),
        vec![],
    )
    .unwrap();
...
fn audit_entries_scoped_to_tenant() {
    ...
    db_a.write_audit("users".into(), "r1".into(), "INSERT".into(), None, None, None).unwrap();
    db_a.write_audit("users".into(), "r2".into(), "INSERT".into(), None, None, None).unwrap();
    db_b.write_audit("users".into(), "r3".into(), "INSERT".into(), None, None, None).unwrap();

    // Each tenant only sees their own entries
    assert_eq!(db_a.get_history("users".into(), "r1".into(), 50).unwrap().len(), 1);
    assert_eq!(db_a.get_history("users".into(), "r3".into(), 50).unwrap().len(), 0); // r3 is tenant_b
    assert_eq!(db_b.get_history("users".into(), "r3".into(), 50).unwrap().len(), 1);
...
fn audit_with_unicode_data() {
    ...
    db.write_audit(
        "users".into(), "r1".into(), "INSERT".into(),
        None, Some("{\"name\":\"café ☕ 日本語\"}".into()), None,
    ).unwrap();

    let h = db.get_history("users".into(), "r1".into(), 1).unwrap();
    assert_eq!(h[0].after_json.as_deref(), Some("{\"name\":\"café ☕ 日本語\"}"));
...
fn audit_with_large_json() {
    ...
    db.write_audit("users".into(), "r1".into(), "INSERT".into(), None, Some(large_json.clone()), None).unwrap();
...
fn audit_checksum_is_64_hex_chars() {
    ...
    db.write_audit("t".into(), "r".into(), "INSERT".into(), None, None, None).unwrap();
...
fn audit_ops_fail_after_close() {
    ...
    assert!(db.write_audit("t".into(), "r".into(), "INSERT".into(), None, None, None).is_err());
